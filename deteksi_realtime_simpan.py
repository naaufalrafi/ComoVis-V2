...
if __name__ == '__main__':
    ...
    # 1. Dapatkan properti video input
    frame_width = int(cap.get(cv2.CAP_PROP_FRAME_WIDTH))
    frame_height = int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))
    fps = cap.get(cv2.CAP_PROP_FPS)

    # 2. Tentukan nama file output dan codec
    base_name = os.path.basename(video_source) # Ambil nama file saja, cth: data_JI.mp4
    name, ext = os.path.splitext(base_name) # Pisahkan nama dan ekstensi, cth: ('data_JI', '.mp4')
    output_filename = f"{name}_hasil_deteksi.mp4"
    ...
    print(f"Hasil video akan disimpan sebagai: {output_filename}")
    ...
    while True:
        ...
        # ### BARU ###: Tulis frame yang sudah diproses ke file video output
    ...
    # ### BARU ###: Lepaskan objek VideoWriter setelah selesai
    out.release()
    # ### AKHIR BARU ###
            
    cap.release()
    cv2.destroyAllWindows()
    print(f"Program dihentikan. Video berhasil disimpan di {output_filename}")